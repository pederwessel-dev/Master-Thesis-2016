...
def logprofiledata(foldername, station_no, files, Nsectors=16):
	...
	bin = 360./Nsectors
	for file in files:
		cols = dataset(path+file,criterion)[0]
		...
		for k in range(len(dircolname)):
			...
			while upper <= 360.:
				...
				x_ar = np.asarray(x_av,float)
				y_ar = np.asarray(y_av,float)					
				logplotter(dirfoldername, counter, direction, dircolname[k], \
				str(station_no), bin/2. ,x_ar, y_ar)
				direction += bin
				lower,upper = direction - bin/2., direction + bin/2.
				counter += 1
	return ""

def logplotter(folder, counter, direction, speeddir, station_no, tolerance, x , y):
	import matplotlib.pyplot as plt
	plt.hold('on')
	titlefont = {'fontname': 'Arial', 'size': '15', 'weight':'normal'}
	axisfont = {'fontname': 'Arial', 'size': '14'}
	fig = plt.figure(counter)
	plt.plot(x , y, color = "black", marker = ".", linestyle = "None", markersize=12)
	plt.title(r"Vertical Profile (dir: %.1f$^\circ$,+/-%.2f)" % (direction,tolerance))
	plt.xlabel("Speed (m/s)")
	plt.ylabel("Height (m)")
	plt.ylim(0,max(y)+10)
	fig.savefig(folder + "/" + "Sector_%d-%s-%s.png" % (counter, station_no, speeddir))
	plt.hold('off')
