...
def Lapsecriteria(precision, tolerance):
    import os
    path_m = "Matrices/"
...
def stats():
    """
    This function initiates generation of results.
    """
    import os,shutil
...
def dataTest():
    """
    This function tests if the iceFiltered data and the data for temperature differences 
    are indexed in the same way.
    """
    path1 = "Matrices"
    path2 = "Matrices-ice_F"
    import os
    for filename1 in os.listdir(path1):
        for filename2 in os.listdir(path2):
            if filename1[0:4] == filename2[0:4]:
                ilist1,jlist1,varlist1 = matrix_reader(path1 + "/" + filename1)
                ilist2, jlist2,varlist2 = matrix_reader(path2 + "/" + filename2)
                for i in range(len(filename1)):
                    if jlist1[i] != jlist2[i]:
                        print filename1[0:4], i
    return "The datasets are synchronized."
...
#    LapseCriteria_writer(date_m,path_n,topfiles,10**-4)
    #Testing the validity of the written matrice

#    cols = dataset(path_n + topfiles[0][1], criterion,startdate="2009-02-23",enddate="2010-02-22")[0]
#    print cols['Temp_T107_A_Avg 58.0'][10]
#    print cols['Temp_termodiff_A_Avg 58.0'][10]
#    print cols['Date Field'][10]
#    print cols['Time Field'][10]
#    print date_m[45]
    #Correlation_Fischer3D_ar(date_m)
