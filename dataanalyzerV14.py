...
def elevraster(linestring, gridresolution, file,data):
    ...
    for distance in arlist:
        ...
        if 23 < data[rasterx,rastery] <128:
            overz.append([xp,yp, data[rasterx,rastery]])
...
def elevation_plotter(path, d,z, dir, st_no1,st_no2,diagonal, counter):
    """
    ...
    params = {'text.latex.preamble' : [r'\usepackage{amsmath}'],'axes.linewidth' : 0.3, 'xtick.labelsize' : 4, 'ytick.labelsize': 4, 'legend.framealpha': 0.7, 'legend.fontsize': 4, 'grid.linewidth': 0.2, 'grid.alpha': 0.6, 'patch.linewidth': 0.3}
    ...
    ax1.set_title(t1+t2, weight = 'semibold', fontsize = 5)
...
def elevation_comb_loop(path):
    ...
    for i in range(len(c)):
        ...
        #rel = [resx_el,resy_el]
        rf = [resx_f, resy_f]
        x_el,y_el,z_el,dist_el, diagonal_el = elevraster(a,rf,set_el,dataset_el)
        ...
        st_no1 = c['M1'][i]
        st_no2 = c['M2'][i]
        elevation_plotter(path, d,z, dir, st_no1,st_no2,dia,i+1)
...
def new_Lapsecriteria_loop(path, precision, tolerance):
    import os,shutil
    # From the function zlisting. The z-values are written to speed up the code.
    infile = open('zlist.txt','r')
    g0 = 9.80665 # m/s2
    ...
    for line in lines:
        ...
        z_dt.append(eval(row[2]))
    critlist_all = []
    foldername = path + "/" + "New_crit_stats_prec_%d_tol_%g" % (precision,tolerance)
    os.mkdir(foldername)
    counter_out = 0
    for i,st in enumerate(st_z):
        filename = foldername + "/" + "%s-lapsecrit-prec_%d-tol_%g.txt" %(st,precision,tolerance)
        wd = new_matrix_reader("wd",st)['wd']
        ws = new_matrix_reader("ws",st)['ws']
        ws_std = new_matrix_reader("ws_std",st)['ws_std']
        p1 = new_matrix_reader("P",st)['P']*100.
        t = new_matrix_reader("t",st)['t']
        date = new_matrix_reader("t",st)['Date']
        time = new_matrix_reader("t",st)['Time']
        dt = new_matrix_reader("dt",st)['dt']
        RH = new_matrix_reader("RH",st)['RH']/100.
        if z_t[i] == 2.0:
            z1 = z_dt[i]
            z2 = z_t[i]
            t1 = t
        ...
        Q1 = 0.622*RH*sat_vap_p_ar("liq",t1)/p1
        Q2 = 0.622*RH*sat_vap_p_ar("liq",t2)/p2
        static_par = np.around(np.array(t1*(1 + 0.611*Q1)-(t2*(1 + 0.611*Q2)))/(z2 - z1),decimals = precision)
            return static_par
        """
        if len(static_par) != 0:
            counter = new_lapse_crit_writer(filename,static_par, wd,ws,ws_std,t,date,time,precision,tolerance)
            counter_out += counter
            new_lapse_crit_writer(filename,static_par, wd,ws,ws_std,t,date,time,precision,tolerance)
    print "The total number of periods with neutral atmosperic stability are: ", counter_out, "and the tolerance is: ", tolerance
    return ""
        """
...
def Newest_matrix_writer_main():
    ...
    """
    path_s_par = path_m + "/"+ "s_parmatrices"
    ...
    for i,st in enumerate(st_z):
        ...
        filename_s_par = path_mp + "/" + "%s-s_parmatrix.txt" % st
        ...
        new_matrix_write_loop(filename_s_par, t_d, t_t, RH_d, RH_t, RH)
        ...
        new_matrix_write_loop(filename_p, t_d, t_t, RH_d, RH_t, RH)
        new_matrix_write_loop(filename_rh, t_d, t_t, p_d, p_t, p)  
...
def elevation_plot_main():
    ...
    foldername = "Elevation-Profiles_new_RT90(25x25)-ylim"
    if os.path.isdir(foldername):
        shutil.rmtree(foldername)
    os.mkdir(foldername)
...
#c = comb_UTM_array()
#for i in range(len(c)):
#    new_combfinder(c['M1'][i],c['M2'][i],c['Dir1'][i],c['Dir2'][i],0,0)
#new_combfinder(6605,85.3,6618,265.3,0,0)
#new_combfinder(6615,109.2,6618,289.2,0,0)
#new_combfinder(6603,157.2,6610,337.2,10,0)
#new_combfinder(6605,318.5,6615,138.5,10,0)
#new_combfinder(6604,128.5,6614,308.5,10,0)
